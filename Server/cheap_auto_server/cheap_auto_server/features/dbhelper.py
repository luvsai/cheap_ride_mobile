...
def readTrip():
    collection_name = dbname["trips"]
    cd = collection_name.find({})
    collectiondata = []
    for r in cd:
        collectiondata.append(r)
    if len(collectiondata) == 0:
        print("Initialiazed trips users db")
        query = {"_id": ObjectId("63b6fb0f95c9095e4d69668e")}
...
def savetrips(trips):
    query = {"_id": ObjectId("63b6fb0f95c9095e4d69668e")}
    collection_name = dbname["trips"]
    result = collection_name.replace_one(query, trips)
# ----below are samples
